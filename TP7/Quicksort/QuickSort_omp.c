...
void QuickSort(int tableau[], int debut, int fin, int prof_max)
{
	int gauche = debut - 1;
	int droite = fin + 1;
	...
	if(prof_max <= 0){
		QuickSort(tableau, debut, droite - 1, 0);
	  
		QuickSort(tableau, droite + 1, fin, 0);
	}else{
	  	#pragma omp task
		QuickSort(tableau, debut, droite - 1, prof_max - 1);
	  
	  	#pragma omp task
		QuickSort(tableau, droite + 1, fin, prof_max - 1);
	}
	  	#pragma omp taskwait
}
...
int main(int argc, char **argv)
{
	...
	{
		#pragma omp single
		QuickSort(tableau, 0, taille - 1, 20);
	}
	/* Arret chrono */
	fin = my_gettimeofday();
	printf
	    ("Pour n=%d (2^n=%d): temps de calcul (avec gettimeofday()) : %g s\n",
	     n, taille, fin - debut);
	...
	for (i = 0; i < taille - 1; i++) {
		if (tableau[i] > tableau[i + 1]) {
			printf
			    ("### Erreur pour tableau[%d] = %d et tableau[%d] = %d \n",
