...
def learnHMM(allx, allq, N, K, initTo1=False):
    """
    Apprend les paramÃ¨tres d'un modÃ¨le HMM par comptage d'une sÃ©rie de sÃ©quences Ã©tiquetÃ©e
    :param allx: observations
    [[obs1, ... , obsT], [obs1, ..., obsT], ...]
         Seq 1                 Seq 2        ...
    :param allq: Ã©tiquetage
    [[s1, ... , sT], [s1, ..., sT], ...]
         Seq 1            Seq 2        ...
    :param N: nombre d'Ã©tats
    :param K: nombre d'observations
    :param initTo1: initialisation Ã  1 (ou epsilon) pour Ã©viter les proba 0
    :return: Pi, A, B
    Les matrices de paramÃ©trage des HMM
    """
    if initTo1:
        eps = 1e-8
        #A = 1-np.tri(N)*eps
...
def calc_log_pobs(x, Pi,A,B):
    """
    Algorithme alpha de calcul de la vraisemblance d'une sÃ©quence d'observations sachant le modÃ¨le
    p(x | lambda)
    :param x: [obs1, ... , obsT] (UNE sÃ©quence)
    :param Pi: param HMM
    :param A: param HMM
    :param B: param HMM
...
def viterbi(x,Pi,A,B):
    """
    Algorithme de Viterbi (en log) pour le dÃ©codage des sÃ©quences d'Ã©tats:
    argmax_s p(x, s | lambda)
    :param x: [obs1, ... , obsT] (UNE sÃ©quence)
    :param Pi: param HMM
    :param A: param HMM
    :param B: param HMM
    :return: s (la sÃ©quence d'Ã©tat la plus probable), estimation de p(x|lambda)
    """
    T = len(x)
    N = len(Pi)
    logA = np.log(A)
