...
int main(int argc, char *argv[]) {
  ...
  if(P != ROOTPROC)
    ima = (unsigned char *)malloc( w*hprim*sizeof(unsigned char));
  else
  ...
  if(P == ROOTPROC)
    for(int k = 0; k < N; ++k){
      if(k != ROOTPROC){
        MPI_Recv(ima + k*w*hprim*sizeof(unsigned char), w*hprim*sizeof(unsigned char), MPI_UNSIGNED_CHAR, k, tag, MPI_COMM_WORLD, &status);
      }
    }
  else
    MPI_Send(ima, w*hprim*sizeof(unsigned char), MPI_UNSIGNED_CHAR, ROOTPROC, tag, MPI_COMM_WORLD);
