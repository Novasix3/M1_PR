...
int main(int argc, char *argv[]) {
  ...
  if(P != ROOTPROC){
    while(1){
      MPI_Recv(&nblock, 1, MPI_INT, ROOTPROC, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
      if(status.MPI_TAG == TAG_END)
        break;

      /* Traitement de la grille point par point */
      y = ymin + nlines*nblock*yinc; 
      for (i = 0; i < nlines; i++) { 
        x = xmin;
        for (j = 0; j < w; j++) {
      ...
      MPI_Send(&nblock, 1, MPI_INT, ROOTPROC, TAG_REQ, MPI_COMM_WORLD);
      MPI_Send(ima, w*nlines, MPI_UNSIGNED_CHAR, ROOTPROC, TAG_DATA, MPI_COMM_WORLD);
    }
  }else{
    int cpt = 0, numblock;
    int sentblock = 0;
    ...
    for(int i = 0; i < N; ++i){
      if(i != ROOTPROC){
        MPI_Send(&sentblock, 1, MPI_INT, i, TAG_REQ, MPI_COMM_WORLD);
        sentblock++;
      }
    }
    ...
    while(cpt < nblock){
      ...
      MPI_Recv(&numblock, 1, MPI_INT, MPI_ANY_SOURCE, TAG_REQ, MPI_COMM_WORLD, &status);
      MPI_Recv(ima + numblock*nlines*w, w*nlines, MPI_UNSIGNED_CHAR, 
        status.MPI_SOURCE, TAG_DATA, MPI_COMM_WORLD, &status);
      cpt++;

      if(sentblock < nblock)
        MPI_Send(&sentblock, 1, MPI_INT, status.MPI_SOURCE, TAG_REQ, MPI_COMM_WORLD);
      else
        MPI_Send(&sentblock, 1, MPI_INT, status.MPI_SOURCE, TAG_END, MPI_COMM_WORLD);
